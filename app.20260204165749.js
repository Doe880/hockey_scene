...
(() => {
  ...
  function updateScoreboard(force=false){
    ...
    if (!force){
      scoreboardEl.style.filter = (performance.now() < state.flashUntil) ? "brightness(1.25)" : "none";
    }
  }
  ...
  function layout(rescale){
    ...
    // Вратари: точно "перед воротами"
    // top: ниже ворот (в поле)
    // bottom: выше ворот (в поле)
    const goalieOffset = Math.max(58, Math.round(rink.h * 0.06)); // больше, чтобы не "внутри"
    goalies.top.y = goals.top.y + goalieOffset;
    goalies.bottom.y = goals.bottom.y - goalieOffset;
    ...
    // === Игроки строго на красной линии (центр катка) ===
    const centerY = rink.y + (rink.h / 2);

    // Чтобы "на линии" стояли коньки, учитываем якорь рисования:
    // в drawPlayer ноги до коньков: (54 + 40) = 94, потом scale
    function yForSkatesOnLine(scale){
      const feet = 94 * scale;
      return centerY - feet;
    }

    // Разнести игроков по X сильнее
    const spread = rink.w * 0.24;

    players[0].x = centerX - spread;
    players[0].y = yForSkatesOnLine(players[0].scale);

    players[1].x = centerX + spread;
    players[1].y = yForSkatesOnLine(players[1].scale);
  ...
  // цели: loko -> нижние, spartak -> верхние
  ...
  function bounceFromGoalie(side){
    ...
    if (side === "top") puck.vy = Math.abs(puck.vy);
    else puck.vy = -Math.abs(puck.vy);
  ...
  function updateShotPuck(dt){
    ...
    if (puck.vy < 0 && puckHitsGoalie("top")) bounceFromGoalie("top");
    else if (puck.vy > 0 && puckHitsGoalie("bottom")) bounceFromGoalie("bottom");
  ...
  function playerAt(x,y){
    ...
    for (const p of players){
      if (Math.hypot(x-p.x, y-p.y) <= pickR) return p;
    }
    return null;
  }
  ...
  function tick(now){
    ...
    if (puck.owner) syncPuckToOwner();
    else updateShotPuck(dt);
