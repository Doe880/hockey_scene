...
(() => {
  ...
  // Ворота рисуем так:
  // - TOP: "лицом вниз" (в сторону игроков)
  // - BOTTOM: "лицом вверх" (в сторону игроков) => зеркалим по Y
  function drawGoalHorizontal(cx, cy, faceDown){
    ctx.save();
    ctx.translate(cx, cy);

    // если ворота должны "смотреть вверх" — зеркалим по Y
    // (faceDown=true => обычная ориентация; faceDown=false => переворот)
  ...
  function tick(now){
    ...
    // TOP ворота "смотрят вниз" (к игрокам)
    drawGoalHorizontal(goals.top.x, goals.top.y, true);
    // BOTTOM ворота "смотрят вверх" (к игрокам) => faceDown=false
