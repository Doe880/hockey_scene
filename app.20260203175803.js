...
(() => {
  ...
  const state = {
    ...
    msg: "",
    msgUntil: 0,
    controlledId: 0, // кто под управлением
  };
  ...
  function getOtherControlled(){ return players.find(p => p.id !== state.controlledId) || players[1]; }
  ...
  function layout(rescale){
    ...
    if (rescale && old){
      ...
      // если ресайз — лучше сбросить текущий жест
      touch.active = false;
      touch.aiming = false;
      touch.id = null;
    }
  ...
  function reset(){
    ...
    puck.active = true;
    puck.owner = players[0]; // стартовое владение у Локо
    puck.trail = [];
    syncPuckToOwner();
  ...
  function drawPlayer(p, t){
    ...
    // индикатор выбранного игрока (обводка)
  ...
  function shootPuck(fromPlayer, tx, ty){
    // если шайбы нет у игрока — но шайба рядом, позволим "подобрать и бросить"
    if (puck.owner !== fromPlayer){
      const d = dist(fromPlayer.x, fromPlayer.y, puck.x, puck.y);
      if (puck.owner && puck.owner !== fromPlayer && d < 110){
        // "перехват" в упрощенном виде
        puck.owner = fromPlayer;
        syncPuckToOwner();
      } else {
  ...
  function checkGoalOrSave(){
    ...
    if (puck.x <= goalLineLeftX){
      ...
      puck.owner = getControlled(); // после эпизода отдаём шайбу текущему игроку
      syncPuckToOwner();
      return true;
    }
  ...
  function goalCenterFor(player){
    // Локомотив атакует вправо, Спартак — влево
    return (player.club.key === "loko") ? goals.right : goals.left;
  }

  function quickShotFrom(player, tapX, tapY){
    // Если ткнули рядом с воротами — целимся туда же, иначе в створ соперника
    const targetGoal = goalCenterFor(player);
    let tx = targetGoal.x;
    let ty = targetGoal.y;

    if (isInsideRink(tapX, tapY)){
      // чуть уважаем место даблтапа (удобно стрелять в угол)
      const towardGoal = (player.club.key === "loko") ? (tapX > player.x) : (tapX < player.x);
      if (towardGoal){
        tx = clamp(tapX, rink.x+8, rink.x+rink.w-8);
        ty = clamp(tapY, rink.y+8, rink.y+rink.h-8);
      } else {
        ty = targetGoal.y + (Math.random()*goals.mouthH - goals.mouthH/2)*0.35;
      }
    }
  ...
  canvas.addEventListener("touchmove", (e) => {
    ...
    // Порог "случайного движения" — побольше, чтобы тапы не срывались
  ...
  canvas.addEventListener("touchend", (e) => {
    ...
    const { x: endX, y: endY } = eventToCanvasXY(e, tmatch);
    const endT = performance.now();
    const heldMs = endT - touch.startT;

    const controlled = getControlled();

    // 1) если был прицел — бросок по прицелу
    if (touch.aiming){
    ...
    // 2) короткий тап (без движения) => либо выбор игрока, либо double tap бросок
    if (!touch.moved && heldMs < 220){
      const dtapMs = endT - touch.lastTapT;
      const dp = dist(endX, endY, touch.lastTapX, touch.lastTapY);

      const isDouble = (dtapMs > 0 && dtapMs < 320 && dp < 46);

      // обновим "последний тап"
      touch.lastTapT = endT;
      touch.lastTapX = endX;
      touch.lastTapY = endY;

      const picked = playerAt(endX, endY);

      if (isDouble){
        // ДВОЙНОЙ ТАП = быстрый бросок
        quickShotFrom(controlled, endX, endY);
      } else {
        // ОДИНАРНЫЙ ТАП = выбор игрока (если тап по нему)
        if (picked){
          state.controlledId = picked.id;
          // по желанию: если шайба у другого игрока и вы выбрали нового — не телепортируем шайбу
          // (пусть останется у владельца). Но если шайба "у вас" — синхронизируем.
          if (puck.owner === picked){
            syncPuckToOwner();
          }
        }
      }
  ...
  canvas.addEventListener("mousedown", (e) => {
    ...
    if (!isInsideRink(x,y)) return;
    mouseStartT = performance.now();
    mouseStartX = x; mouseStartY = y;
    mouseMoved = false;
    mouseAiming = false;
    mouseAimX = x; mouseAimY = y;
  });

  window.addEventListener("mousemove", (e) => {
    if (!mouseDown) return;
    const {x,y} = eventToCanvasXY(e);
    if (dist(mouseStartX, mouseStartY, x, y) > 28) mouseMoved = true;
    if (performance.now() - mouseStartT > 180){
      mouseAiming = true;
      mouseAimX = clamp(x, rink.x+8, rink.x+rink.w-8);
      mouseAimY = clamp(y, rink.y+8, rink.y+rink.h-8);
    }
    touch.x = x; touch.y = y; // чтобы движение работало одинаково
    touch.active = true;
  });
  ...
  window.addEventListener("mouseup", (e) => {
    ...
    const {x,y} = eventToCanvasXY(e);
    const endT = performance.now();
    const heldMs = endT - mouseStartT;
    const controlled = getControlled();

    if (mouseAiming){
      shootPuck(controlled, mouseAimX, mouseAimY);
    } else if (!mouseMoved && heldMs < 220){
      const dtapMs = endT - mouseLastTapT;
      const dp = dist(x,y, mouseLastTapX, mouseLastTapY);
      const isDouble = (dtapMs > 0 && dtapMs < 320 && dp < 46);
      mouseLastTapT = endT; mouseLastTapX = x; mouseLastTapY = y;

      const picked = playerAt(x,y);
      if (isDouble){
        quickShotFrom(controlled, x, y);
      } else if (picked){
        state.controlledId = picked.id;
  ...
  function updateAIPlayer(p, dt, t){
    // AI для "неуправляемого" игрока: чуть патрулит и реагирует на шайбу
