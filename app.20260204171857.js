...
(() => {
  ...
  function layout(rescale){
    ...
    // игроки на красной линии — коньки на centerY
  ...
  function shootBySwipe(fromPlayer, sx, sy, ex, ey){
    ...
    if (swipeLen < 70) return;

    const target = (fromPlayer.club.key === "loko") ? goals.bottom : goals.top;
    const speed = Math.max(520, Math.min(1300, swipeLen * 14.0));
  ...
  function bounceFromGoalie(side){
    ...
    const dx = puck.x - b.x;
    const sideKick = Math.max(-520, Math.min(520, dx * 16));
